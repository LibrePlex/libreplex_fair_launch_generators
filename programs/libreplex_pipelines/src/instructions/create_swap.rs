...
use libreplex_monoswap_deprecated::{cpi::accounts::CreateMonoSwapCtx, CreateMonoSwapInput};
...
pub struct CreateSwapCtx<'info> {
    ...
    #[account(mut,
        ...
        seeds::program = libreplex_monoswap_deprecated::ID,
    bump)]
    ...
    */
    ...
    /// CHECK: ID checked in constraint
    #[account(
        constraint = libreplex_monoswap_program.key() == libreplex_monoswap_deprecated::ID
    )]
...
pub fn create_swap(ctx: Context<CreateSwapCtx>, input: FilterInput) -> Result<()> {
    ...
    libreplex_monoswap_deprecated::cpi::create_monoswap(
